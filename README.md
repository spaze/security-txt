...
The messages in the exceptions as thrown by this library do not contain any sensitive information, but they are not sanitized: `getMessage()` returns the server-supplied values as they arrived, control characters and all, so a checked host can move a terminal's cursor or reverse what follows with a bidirectional override. `bin/checksecuritytxt.php` does not have that problem because `ConsolePrinter` encodes everything down to printable ASCII before printing it, and if you display messages yourself you need to do the same. 3.x does it inside `getMessage()` instead, so it is safe to display there.
The same goes for HTML: do not display the messages as HTML and do not feed them into a Markdown parser or similar.
