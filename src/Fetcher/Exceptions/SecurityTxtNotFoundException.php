...
final class SecurityTxtNotFoundException extends SecurityTxtFetcherException
{
	...
	/**
	 * @param non-empty-array<string, array{0:string, 1:1|134217728, 2:int, 3:list<string>, 4:bool, 5:bool}> $urls URL => 0: IP address, 1: DNS record type, 2: HTTP code, 3: redirects, 4: is regular HTML page?, 5: response too long?
	...
	public function __construct(array $urls, ?Throwable $previous = null)
	{
		...
		foreach ($urls as $url => $components) {
			...
			if ($components[3] !== []) {
				...
				$message .= $components[4] || $components[5] ? ' (final page after redirects)' : ' (final code after redirects)';
			}
		}
