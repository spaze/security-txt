...
final class SecurityTxtWriterParseTest extends TestCase
{
	...
	public function testWriteParse(): void
	{
		...
		$securityTxt->addAcknowledgments(new Acknowledgments('https://ack2.example'));
		$securityTxt->addCanonical(new Canonical('https://canonical1.example'));
		$securityTxt->addCanonical(new Canonical('https://canonical2.example'));
		...
		$securityTxt->addEncryption(new Encryption('https://encryption1.example'));
		$securityTxt->addEncryption(new Encryption('https://encryption2.example'));
		$securityTxt->setExpires(new Expires($dateTime));
		$securityTxt->addHiring(new Hiring('https://hiring1.example'));
		$securityTxt->addHiring(new Hiring('https://hiring2.example'));
		$securityTxt->addPolicy(new Policy('https://policy1.example'));
		$securityTxt->addPolicy(new Policy('https://policy2.example'));
		$securityTxt->setPreferredLanguages(new PreferredLanguages(['en', 'cs']));
