...
final class SecurityTxtFetcherCurlClientNoExtensionTest extends TestCase
{
	...
	public function testExceptionWhenExtensionNotLoaded(): void
	{
		...
		Assert::throws(function () use ($client) {
			$client->getResponse(new SecurityTxtFetcherUrl(new Url('https://example.com/'), new SecurityTxtRedirects()), SecurityTxtHost::fromString('example.com'), '192.0.2.1', SecurityTxtIpAddressType::V4);
