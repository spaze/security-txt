...
final class SecurityTxtCannotOpenUrlExceptionTest extends TestCase
{
	...
	{
		$exception = new SecurityTxtCannotOpenUrlException(new Url('https://com.example/'), []);
		...
		$exception = new SecurityTxtCannotOpenUrlException(new Url('https://com.example/'), [], '192.0.2.1', SecurityTxtIpAddressType::V4);
		...
		$exception = new SecurityTxtCannotOpenUrlException(new Url('https://com.example/'), [], '2001:DB8::1', SecurityTxtIpAddressType::V6, 'Could not connect to server');
		...
		$exception = new SecurityTxtCannotOpenUrlException(new Url("https://evil.example/\x1b[2K"), []);
		...
		$exception = new SecurityTxtCannotOpenUrlException(new Url('https://com.example/'), ['https://redir1.example/']);
		Assert::same("Can't open https://com.example/ (redirects: https://redir1.example/)", $exception->getMessage());

		$exception = new SecurityTxtCannotOpenUrlException(new Url('https://com.example/'), [], '192.0.2.1');
